...
function LettersBar({handleSelectLetter}) {
  ...
  return (
        <ul className='grid grid-cols-13 place-items-center 
        ...
        md:flex md:justify-around'>
            ...
            lettersArray.map((letter) => {
                return  <li className='text-white 
                            font-Inter text-xl' 
                            key={letter}>
                                <button onClick={()=> handleSelectLetter(letter)} className=' w-6 p-1 text-center
                                      hover:bg-green-500 hover:shadow-light
                                      hover:rounded-t-md hover:-translate-y-1 hover:duration-300
